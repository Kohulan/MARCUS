...
export default {
  ...
  state: {
    ...
    processingOptions: {      // Options for structure processing
  ...
  actions: {
    ...
    async fetchSegments({ commit, dispatch, state }, pdfFile, options = {}) {
      ...
      try {
        ...
        const response = await decimerService.extractSegments(pdfFile, {
          collectAll: options.collectAll !== undefined ? options.collectAll : true
        })
        ...
        if (response && response.segments_count) {
          ...
          try {
            ...
            if (segments.length === 0 && response.segments_count > 0) {
              console.warn('No segments returned from listSegmentFiles despite segments_count > 0')
              commit('SET_ERROR', 'No segment files found despite successful extraction')
            }
          } catch (listError) {
            console.error('Error listing segment files:', listError)
            commit('SET_ERROR', `Error listing segment files: ${listError.message}`)
          }
    ...
    async processStructures({ commit, state, dispatch }, options = {}) {
      ...
      if (options.model) {
        ...
        const engine = validEngines.includes(options.model) ? options.model : 'decimer';
        
        // Auto-enable includeMolfile for molnextr and molscribe engines
        const includeMolfile = options.includeMolfile || 
                             (engine === 'molnextr' || engine === 'molscribe');
      ...
      try {
        ...
        console.log(`Processing ${state.segments.length} segments with options:`, processingOptions)
        ...
        if (process.env.NODE_ENV === 'development' && process.env.VUE_APP_USE_MOCK_DATA === 'true') {
          ...
          results = state.segments.map(segment => ({
          ...
          const segmentsToProcess = options.segments || state.segments;
          
          // Call the real OCSR service
          results = await ocsrService.processSegments(segmentsToProcess, processingOptions);
        }
        
        // Format and store the results
        const convertedStructures = results.map(result => {
          // Extract segment ID from the path if not available directly
          const segmentId = result.segmentId || `segment-${result.image_name?.split('_')[2] || '0'}`
          ...
          return {
            ...
            uniqueKey: result.uniqueKey || `pdf-${state.currentPdfId}-${segmentId}`,
            // Use the result's name or fallback to a default - FIX FOR UNDEFINED SEGMENT
            name: result.name || result.filename || (segmentId.split('-')[1] ? `Compound ${segmentId.split('-')[1]}` : 'Unknown Compound'),
  ...
  getters: {
    ...
    hasConvertedStructures: state => state.convertedStructures && state.convertedStructures.length > 0,
    segmentCount: state => state.segments ? state.segments.length : 0,
    convertedCount: state => state.convertedStructures ? state.convertedStructures.length : 0,
    ...
    getStructureForSegment: state => segmentId => {
      ...
      if (state.processedStructuresCache && state.processedStructuresCache.has(segmentId)) {
        const cachedStructure = state.processedStructuresCache.get(segmentId);
        // Only return if it matches the current PDF
        if (!state.currentPdfId || cachedStructure.pdfId === state.currentPdfId) {
    ...
    getStructureByFilename: state => filename => {
      ...
      if (state.processedStructuresCache && state.processedStructuresCache.has(filename)) {
    ...
    getStructureByUniqueKey: state => uniqueKey => {
      ...
      // Try to look up in our cache first
      if (state.processedStructuresCache && state.processedStructuresCache.has(uniqueKey)) {
