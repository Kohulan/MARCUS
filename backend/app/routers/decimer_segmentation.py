...
from fastapi import APIRouter, Body, HTTPException, Query, status, UploadFile, File, Form
...
async def extract_segments(
    pdf_file: UploadFile = File(...),
    collect_all: bool = Form(True, description="Whether to collect all segments in a common directory"),
):
    """
    ...
    try:
        ...
        if collect_all and "all_segments_directory" in segments_result and segments_result["all_segments_directory"]:
        ...
        if "all_segments_directory" in segments_result and segments_result["all_segments_directory"]:
            try:
                ...
                all_segments_rel_path = os.path.basename(segments_result["all_segments_directory"])
        ...
        for item in os.listdir(SEGMENTS_DIR):
            ...
            if os.path.isdir(item_path):
                ...
                if os.path.exists(all_segments_dir):
                    segments_count = len([f for f in os.listdir(all_segments_dir) 
...
async def get_segment_image(
    ...
    try:
        ...
        if "all_segments" in image_name:
            ...
            image_path = os.path.join(SEGMENTS_DIR, directory, "all_segments", image_name)
        else:
...
def _count_segments(directory_path: str) -> int:
    ...
    try:
        ...
        if os.path.basename(directory_path) == "all_segments" and os.path.exists(directory_path):
            return len([f for f in os.listdir(directory_path) if f.endswith('_segmented.png')])
        ...
        if os.path.exists(all_segments_dir) and os.path.isdir(all_segments_dir):
            return len([f for f in os.listdir(all_segments_dir) if f.endswith('_segmented.png')])
        ...
        for item in os.listdir(directory_path):
            ...
            if item.endswith('_segments') and os.path.isdir(segment_dir_path):
                count += len([f for f in os.listdir(segment_dir_path) 
                             if f.endswith('_segmented.png')])
        return count
    except Exception as e:
