...
from app.modules.ocsr_wrapper import (
    process_chemical_structure,
    save_uploaded_image
)
...
async def generate_smiles(
    image_file: Optional[UploadFile] = File(None, description="Chemical structure image file"),
    image_path: Optional[str] = Form(None, description="Path to existing chemical structure image"),
    engine: Literal["decimer", "molnextr", "molscribe"] = Form("decimer", description="OCSR engine to use"),
    hand_drawn: bool = Form(False, description="Whether to use the hand-drawn model (only for DECIMER)")
):
    """
    ...
    try:
        ...
        if image_file is not None:
            ...
            if not any(filename.endswith(ext) for ext in ['.png', '.jpg', '.jpeg', '.tif', '.tiff', '.bmp']):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
...
async def generate_molfile(
    image_file: Optional[UploadFile] = File(None, description="Chemical structure image file"),
    image_path: Optional[str] = Form(None, description="Path to existing chemical structure image"),
    engine: Literal["molnextr", "molscribe"] = Form("molnextr", description="OCSR engine to use")
):
    """
...
async def generate_both(
    image_file: Optional[UploadFile] = File(None, description="Chemical structure image file"),
    image_path: Optional[str] = Form(None, description="Path to existing chemical structure image"),
    engine: Literal["decimer", "molnextr", "molscribe"] = Form("molnextr", description="OCSR engine to use"),
    hand_drawn: bool = Form(False, description="Whether to use the hand-drawn model (only for DECIMER)")
):
    """
...
async def generate_with_depiction(
    ...
    image_path: Optional[str] = Form(None, description="Path to existing chemical structure image"),
    engine: Literal["decimer", "molnextr", "molscribe"] = Form("molnextr", description="OCSR engine to use"),
    output_type: Literal["smiles", "molfile", "both"] = Form("both", description="Type of chemical notation to generate"),
    hand_drawn: bool = Form(False, description="Whether to use the hand-drawn model (only for DECIMER)"),
    depict_engine: Literal["cdk", "rdkit"] = Form("rdkit", description="Depiction engine to use"),
    ...
    depict_format: Literal["svg", "png", "base64"] = Form("svg", description="Output format for depiction")
):
    """
    ...
    try:
        ...
        if depict_format == "svg":
            ...
            result["depiction"]["base64"] = base64.b64encode(depiction_result["depiction"]).decode("utf-8")
        else:  # already base64
