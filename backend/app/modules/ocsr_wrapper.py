...
def get_decimer_prediction(
    path: Union[str, Path], hand_drawn: bool = False
) -> Tuple[str, str]:
    """
...
def get_molnextr_prediction(
    ...
    if data_type not in ("smiles", "predicted_molfile", "both"):
        raise ValueError(
            "Invalid data_type. Choose 'smiles', 'predicted_molfile', or 'both'."
        )

    # Use the singleton implementation to get predictions
    prediction = molnextr_get_predictions(
        str(path), atoms_bonds=False, smiles=get_smiles, predicted_molfile=get_molfile
    )
...
def get_molscribe_prediction(
    path: Union[str, Path], data_type: Literal["smiles", "molfile", "both"] = "both"
) -> Union[Tuple[str, str], Tuple[str, str, str]]:
    """
...
def get_smiles_ocsr(
    ...
    if engine not in ("decimer", "molnextr", "molscribe"):
        raise ValueError(
            "Invalid engine. Choose 'decimer', 'molnextr', or 'molscribe'."
        )

    if coordinates and engine == "decimer":
        raise ValueError(
            "Coordinates are only available when using the 'molnextr' or 'molscribe' engines."
        )
    ...
    try:
        ...
        if engine == "molscribe":
            if output_type == "smiles":
                image_name, smiles = get_molscribe_prediction(
                    file_path, data_type="smiles"
                )
                return {
                ...
                image_name, molfile = get_molscribe_prediction(
                    file_path, data_type="molfile"
                )
                return {
                    "image_name": image_name,
                    "molfile": molfile,
                ...
                image_name, smiles, molfile = get_molscribe_prediction(
                    file_path, data_type="both"
                )
                return {
                    "image_name": image_name,
                    "smiles": smiles,
        ...
        # Original DECIMER processing
            ...
            if output_type == "smiles":
                image_name, smiles = get_decimer_prediction(
                    file_path, hand_drawn=hand_drawn
                )
                return {
                    "image_name": image_name,
                    "smiles": smiles,
                ...
                raise ValueError(
                    "Molfile output is only available with the 'molnextr' and 'molscribe' engines"
                )
            else:  # output_type == "both"
                image_name, smiles = get_decimer_prediction(
                    file_path, hand_drawn=hand_drawn
        ...
        # Original MolNexTR processing
            ...
            if output_type == "smiles":
                image_name, smiles = get_smiles_ocsr(
                    file_path, engine=engine, coordinates=False
                )
                ...
                image_name, molfile = get_molnextr_prediction(
                    file_path, data_type="predicted_molfile"
                )
                result = molnextr_get_predictions(
                    str(file_path), predicted_molfile=True
                )
                return {
                    "image_name": image_name,
                    "molfile": molfile,
                ...
                image_name, smiles, molfile = get_molnextr_prediction(
                    file_path, data_type="both"
                )
                result = molnextr_get_predictions(
                    str(file_path), smiles=True, predicted_molfile=True
                )
                return {
                    "image_name": image_name,
                    "smiles": smiles,
            ...
            raise ValueError(
                f"Unsupported engine: {engine}. Choose 'decimer', 'molnextr', or 'molscribe'."
            )
