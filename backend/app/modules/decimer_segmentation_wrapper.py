...
def get_segments_with_bbox(file_path: str, poppler_path=None) -> Tuple[str, List[Dict[str, Any]]]:
    """
    ...
    for page_num, page_img in enumerate(images):
        try:
            ...
            segments, bboxes = segment_chemical_structures(page_img, expand=True, return_bboxes=True)
...
def get_complete_segments(path_to_pdf: str, collect_all: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if segments_already_exist:
            ...
            segment_directory, segments_metadata = get_segments_with_bbox(path_to_pdf, poppler_path=None)
...
def get_highlighted_segment_image(
    segment_id: str, 
    pdf_filename: str
) -> str:
    """
...
def segments_exist(filepath: str) -> Tuple[bool, Optional[str]]:
    ...
    matching_dirs = [d for d in os.listdir(SEGMENTS_DIR) 
                    if d.startswith(file_stem) and 
                    os.path.isdir(os.path.join(SEGMENTS_DIR, d))]
